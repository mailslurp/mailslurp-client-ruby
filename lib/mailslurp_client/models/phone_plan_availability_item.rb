  ...
  class PhonePlanAvailabilityItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MailSlurpClient::PhonePlanAvailabilityItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MailSlurpClient::PhonePlanAvailabilityItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @availability_status.nil?
        invalid_properties.push('invalid value for "availability_status", availability_status cannot be nil.')
      end
    ...
    def valid?
      ...
      return false unless availability_status_validator.valid?(@availability_status)
      true
    end
    ...
    def availability_status=(availability_status)
      validator = EnumAttributeValidator.new('String', ["AVAILABLE", "NON_MATCHING_SUBSCRIPTION_CURRENCY", "NON_MATCHING_SUBSCRIPTION_TYPE", "ALREADY_ASSIGNED"])
      unless validator.valid?(availability_status)
        fail ArgumentError, "invalid value for \"availability_status\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [phone_country, availability_status, variants].hash
    end
