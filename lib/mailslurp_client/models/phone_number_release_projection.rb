  ...
  class PhoneNumberReleaseProjection
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MailSlurpClient::PhoneNumberReleaseProjection` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MailSlurpClient::PhoneNumberReleaseProjection`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'subscription_schedule')
        self.subscription_schedule = attributes[:'subscription_schedule']
      end
    ...
    def subscription_schedule=(subscription_schedule)
      validator = EnumAttributeValidator.new('String', ["MONTHLY", "YEARLY"])
      unless validator.valid?(subscription_schedule)
        fail ArgumentError, "invalid value for \"subscription_schedule\", must be one of #{validator.allowable_values}."
      end
      @subscription_schedule = subscription_schedule
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] plan_currency Object to be assigned
    def plan_currency=(plan_currency)
      validator = EnumAttributeValidator.new('String', ["EUR", "USD", "JPY", "GBP", "AUD", "CAD", "CHF", "CNY", "INR", "NZD"])
      unless validator.valid?(plan_currency)
        fail ArgumentError, "invalid value for \"plan_currency\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [user_id, created_at, phone_number, phone_country, subscription_schedule, plan_currency, name, id].hash
    end
