  ...
  class CreateInboxRetentionPolicyForAccountOptions
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MailSlurpClient::CreateInboxRetentionPolicyForAccountOptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MailSlurpClient::CreateInboxRetentionPolicyForAccountOptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @retention_days > 730
        invalid_properties.push('invalid value for "retention_days", must be smaller than or equal to 730.')
      end
    ...
    def valid?
      ...
      return false if @retention_days > 730
      return false if @retention_days < 1
      true
    end
    ...
    def retention_days=(retention_days)
      ...
      if retention_days > 730
        fail ArgumentError, 'invalid value for "retention_days", must be smaller than or equal to 730.'
      end
