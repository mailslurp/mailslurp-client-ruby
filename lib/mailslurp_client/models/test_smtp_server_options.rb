  ...
  class TestSmtpServerOptions
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MailSlurpClient::TestSmtpServerOptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MailSlurpClient::TestSmtpServerOptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'use_start_tls')
        self.use_start_tls = attributes[:'use_start_tls']
      end
    ...
    def list_invalid_properties
      ...
      if @use_start_tls.nil?
        invalid_properties.push('invalid value for "use_start_tls", use_start_tls cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @use_start_tls.nil?
      true
    end
    ...
    def hash
      [host, port, use_start_tls, username, password, from, to, captcha_token].hash
    end
