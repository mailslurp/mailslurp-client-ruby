...
module MailSlurpClient
  class DomainMonitorRunDueResult
    ...
    attr_accessor :requested_max_runs
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MailSlurpClient::DomainMonitorRunDueResult` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MailSlurpClient::DomainMonitorRunDueResult`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def trigger_source=(trigger_source)
      validator = EnumAttributeValidator.new('String', ["API", "USER_SCHEDULE", "CRON", "SCHEDULER"])
      unless validator.valid?(trigger_source)
        fail ArgumentError, "invalid value for \"trigger_source\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [trigger_source, run_count, requested_max_runs, executed_at].hash
    end
