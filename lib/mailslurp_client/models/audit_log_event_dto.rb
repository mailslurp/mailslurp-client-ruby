  ...
  class AuditLogEventDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MailSlurpClient::AuditLogEventDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MailSlurpClient::AuditLogEventDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @event_ts.nil?
        invalid_properties.push('invalid value for "event_ts", event_ts cannot be nil.')
      end
    ...
    def hash
      [tenant_id, dt, event_id, event_ts, action, user_id, actor_user_id, target_user_id, resource_type, resource_id, outcome, request_id, ip_address, metadata_json].hash
    end
