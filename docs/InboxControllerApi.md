...
begin
  #Cancel a scheduled email job
  result = api_instance.cancel_scheduled_job(job_id)
  p result
rescue MailSlurpClient::ApiError => e
  puts "Exception when calling InboxControllerApi->cancel_scheduled_job: #{e}"
end
```
...
opts = {
  ...
  use_short_address: true # Boolean | Use a shorter email address under 31 characters
}
...
> InboxExistsDto does_inbox_exist(email_address, opts)
...
opts = {
  allow_catch_all: true # Boolean | 
}

begin
  #Does inbox exist
  result = api_instance.does_inbox_exist(email_address, opts)
  p result
rescue MailSlurpClient::ApiError => e
...
begin
  #Get a scheduled email job
  result = api_instance.get_scheduled_job(job_id)
  p result
rescue MailSlurpClient::ApiError => e
  puts "Exception when calling InboxControllerApi->get_scheduled_job: #{e}"
end
```
...
> ScheduledJobDto send_with_schedule(inbox_id, send_email_options, opts)
...
begin
  #Send email with with delay or schedule
  result = api_instance.send_with_schedule(inbox_id, send_email_options, opts)
  p result
rescue MailSlurpClient::ApiError => e
