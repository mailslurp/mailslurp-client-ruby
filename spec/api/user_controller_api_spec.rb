=begin
#MailSlurp API

#MailSlurp is an API for sending and receiving emails and SMS from dynamically allocated email addresses and phone numbers. It's designed for developers and QA teams to test applications, process inbound emails, send templated notifications, attachments, and more.  ## Resources  - [Homepage](https://www.mailslurp.com) - Get an [API KEY](https://app.mailslurp.com/sign-up/) - Generated [SDK Clients](https://docs.mailslurp.com/) - [Examples](https://github.com/mailslurp/examples) repository

The version of the OpenAPI document: 6.5.2
Contact: contact@mailslurp.dev
Generated by: https://openapi-generator.tech
OpenAPI Generator version: 4.3.1

=end

require 'spec_helper'
require 'json'

# Unit tests for MailSlurpClient::UserControllerApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'UserControllerApi' do
  before do
    # run before each test
    @api_instance = MailSlurpClient::UserControllerApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of UserControllerApi' do
    it 'should create an instance of UserControllerApi' do
      expect(@api_instance).to be_instance_of(MailSlurpClient::UserControllerApi)
    end
  end

  # unit tests for create_or_update_inbox_retention_policy_for_account
  # Create inbox retention policy for your global account
  # @param create_inbox_retention_policy_for_account_options 
  # @param [Hash] opts the optional parameters
  # @return [InboxRetentionPolicyDto]
  describe 'create_or_update_inbox_retention_policy_for_account test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for delete_inbox_retention_policy_for_account
  # Delete inbox retention policy for your global account
  # @param [Hash] opts the optional parameters
  # @return [EmptyResponseDto]
  describe 'delete_inbox_retention_policy_for_account test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for get_entity_automations
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :page Optional page index
  # @option opts [Integer] :size Optional page size
  # @option opts [String] :sort Optional createdAt sort direction ASC or DESC
  # @option opts [DateTime] :since Filter by created at after the given timestamp
  # @option opts [DateTime] :before Filter by created at before the given timestamp
  # @option opts [String] :inbox_id Optional inbox ID
  # @option opts [String] :phone_id Optional phone ID
  # @option opts [String] :filter Optional automation type filter
  # @return [PageEntityAutomationItems]
  describe 'get_entity_automations test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for get_entity_events
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :page Optional page index
  # @option opts [Integer] :size Optional page size
  # @option opts [String] :sort Optional createdAt sort direction ASC or DESC
  # @option opts [DateTime] :since Filter by created at after the given timestamp
  # @option opts [DateTime] :before Filter by created at before the given timestamp
  # @option opts [String] :inbox_id Optional inbox ID
  # @option opts [String] :email_id Optional email ID
  # @option opts [String] :phone_id Optional phone ID
  # @option opts [String] :sms_id Optional SMS ID
  # @option opts [String] :attachment_id Optional attachment ID
  # @option opts [String] :filter Optional type filter
  # @return [PageEntityEventItems]
  describe 'get_entity_events test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for get_entity_favorites
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :page Optional page index
  # @option opts [Integer] :size Optional page size
  # @option opts [String] :sort Optional createdAt sort direction ASC or DESC
  # @option opts [DateTime] :since Filter by created at after the given timestamp
  # @option opts [DateTime] :before Filter by created at before the given timestamp
  # @option opts [String] :filter Optional type filter
  # @return [PageEntityFavouriteItems]
  describe 'get_entity_favorites test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for get_inbox_retention_policy_for_account
  # Get inbox retention policy for your global account
  # @param [Hash] opts the optional parameters
  # @return [InboxRetentionPolicyOptionalDto]
  describe 'get_inbox_retention_policy_for_account test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for get_json_property_as_string
  # Utility function to extract properties from JSON objects in language where this is cumbersome.
  # @param property JSON property name or dot separated path selector such as &#x60;a.b.c&#x60;
  # @param body 
  # @param [Hash] opts the optional parameters
  # @return [String]
  describe 'get_json_property_as_string test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for get_user_info
  # Get account information for your user
  # @param [Hash] opts the optional parameters
  # @return [UserInfoDto]
  describe 'get_user_info test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

end
